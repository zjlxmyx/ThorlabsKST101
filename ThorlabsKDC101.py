...
lib = ctypes.cdll.LoadLibrary(r'C:\Program Files\Thorlabs\Kinesis\Thorlabs.MotionControl.KCube.DCServo.dll')
...
class Motor:
    ...
    def start_polling(self, ms):
        err = lib.CC_StartPolling(self.SN, ms)
        if not err:
    ...
    def stop_polling(self):
        err = lib.CC_StopPolling(self.SN)
        if not err:
    ...
    def set_vel_params(self, Acce, MaxV):
        err = lib.CC_SetVelParams(self.SN, Acce, MaxV)
        if err:
    ...
    def get_vel_params(self):
        err = lib.CC_GetVelParams(self.SN, ctypes.byref(self.Acce_c), ctypes.byref(self.MaxV_c))
        if err:
    ...
    def set_jog_vel_params(self, JogAcce, JogMaxV):
        err = lib.CC_SetJogVelParams(self.SN, JogAcce, JogMaxV)
        if err:
    ...
    def get_jog_vel_params(self):
        err = lib.CC_GetJogVelParams(self.SN, ctypes.byref(self.JogAcce_c), ctypes.byref(self.JogMaxV_c))
        if err:
    ...
    def set_jog_mode(self, jogmode, stopmode):
        err = lib.CC_SetJogMode(self.SN, jogmode, stopmode)
        if err:
    ...
    def get_jog_mode(self):
        err = lib.CC_GetJogMode(self.SN, ctypes.byref(self.JogMode_c), ctypes.byref(self.StopMode_c))
        if err:
    ...
    def set_jog_step_size(self, stepsize):
        err = lib.CC_SetJogStepSize(self.SN, stepsize)
        if err:
    ...
    def get_jog_step_size(self):
        err = lib.CC_GetJogStepSize(self.SN)
        if err:
    ...
    def move_to_position(self, Position):
        err = lib.CC_MoveToPosition(self.SN, Position)
        if err:
    ...
    def move_at_velocity(self, direction):
        err = lib.CC_MoveAtVelocity(self.SN, direction)
        if err:
    ...
    def stop_profiled(self):
        err = lib.CC_StopProfiled(self.SN)
        if err:
    ...
    def get_position(self):
        return lib.CC_GetPosition(self.SN)

    #check if the motor is moving
    # Returns 1 for moving, 0 for stationary
    def is_moving(self):
        bits = ctypes.wintypes.DWORD(lib.CC_GetStatusBits(self.SN))
